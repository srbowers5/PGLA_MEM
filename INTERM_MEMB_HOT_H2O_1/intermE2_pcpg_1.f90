...
integer, parameter :: not_now=1
 ...
 integer, parameter :: na=25184,nmon=21,nmol=2,npmon=nmon*nmol, nlip=98, first_pep=11299, &
                       lipidLeaf=(nlip/2), molLeaf=(nmol/2), &
...
!...nha - # of any H atoms in all peptides
 ...
 real, dimension(numstr,npmon,3) :: term_com(numstr,8)
 ...
 real :: nterm1_com, nterm2_com, cterm1_com, cterm2_com, all1_com, all2_com
 integer :: nterm1_cnt, nterm2_cnt, cterm1_cnt, cterm2_cnt, all1_cnt, all2_cnt
...
! do rep=rep1,rep2
  ...
  if (runnum < 10) then
   ...
   open(55,file=trim(path)//'interm_out/lip_coord_num'//char(48+tr)//'_'//trim(repString)//'_'//trim(runString)//'.dat')
   open(56,file=trim(path)//'interm_out/lip_coord_num_PC_group'//char(48+tr)//'_'//trim(repString)//'_'//trim(runString)//'.dat')
   open(57,file=trim(path)//'interm_out/lip_coord_num_PG_group'//char(48+tr)//'_'//trim(repString)//'_'//trim(runString)//'.dat')
...
!                    print*,m, aanum, "cterm1", stpi%pos(3), cterm1_com, cterm1_cnt, cterm1_com/cterm1_cnt
         ...
         term_com(str,7) = all1_com/all1_cnt
         term_com(str,8) = all2_com/all2_cnt
...
subroutine structure
 ...
 integer, dimension(nmol,nlip) :: lipCoord
 integer, dimension(nmol,nlip,5) :: lipCoord_group
 integer :: lleaf, mleaf, molNum
 integer :: PCcoordRef, PGcoordRef, PCcoordOpp, PGcoordOpp
 integer, dimension(5) :: PCcoordRefGroup, PGcoordRefGroup, PCcoordOppGroup, PGcoordOppGroup
...
!....protein-ligand interactions ...
     ...
     do i=1,npmon
       ...
       do j=1,nlip
         do k=1,5
           if (cmlp(i,j,k) == 1) then
             lipCoord(molNum, j) = 1
             lipCoord_group(molNum, j, k) = 1
           endif
         enddo
       enddo
     enddo
         ...
         if (lipCoord(i, j) == 1) then
           ...
           if (lleaf == mleaf) then
             if  (isPC(j) .eqv. .true.) then
               PCcoordRef = PCcoordRef + 1
               do k=1,5
                   if (lipCoord_Group(i, j, k) == 1) then
                       PCcoordRefGroup(k) = PCcoordRefGroup(k) + 1
                   endif
               enddo
             else
               PGcoordRef = PGcoordRef + 1
               do k=1,5
                   if (lipCoord_Group(i, j, k) == 1) then
                       PGcoordRefGroup(k) = PGcoordRefGroup(k) + 1
                   endif
               enddo
             endif
           else
             if  (isPC(j) .eqv. .true.) then
               do k=1,5
                   if (lipCoord_Group(i, j, k) == 1) then
                       PCcoordOppGroup(k) = PCcoordOppGroup(k) + 1
                   endif
               enddo
               PCcoordOpp = PCcoordOpp + 1
             else
               do k=1,5
                   if (lipCoord_Group(i, j, k) == 1) then
                       PGcoordOppGroup(k) = PGcoordOppGroup(k) + 1
                   endif
               enddo
               PGcoordOpp = PGcoordOpp + 1
             endif
           endif
         endif
       enddo
...
!   distKD(2) = sqrt( sum( (poss(str,45,:)-poss(str,50,:))**2 ) )
   ...
   write(54,'(i6,6(1x,f7.3))')stri,term_com(str,1:8)-z0(str)
   
   write(55,'(i6,4(i4))') PCcoordRef, PGcoordRef, PCcoordOpp, PGcoordOpp
   write(56,'(i6,5(i4),5(i4))') PCcoordRefGroup(:), PCcoordOppGroup(:)
   write(57,'(i6,5(i4),5(i4))') PGcoordRefGroup(:), PGcoordOppGroup(:)
 enddo 
