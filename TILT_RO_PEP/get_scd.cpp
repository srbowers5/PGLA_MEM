...
void write_scd_vals(ReadPdb::LIPID_C_TAIL_PTR lip_c_ptr, double scdValue, int  lipReg, int lipDistInt) {
   ...
   // printf("Xwrite \n");
    fflush(stdout);
   // printf("Xwrite %d %d\n", lip_c_ptr->chain_num-1,lip_c_ptr->chain_pos-1);
    fflush(stdout);
    if (lip_c_ptr->mon_type == ReadPdb::IS_DMPC) {
   // printf("XDMPC %d %d %f\n",lip_c_ptr->chain_num-1, lip_c_ptr->chain_pos-1, scdValue );
    fflush(stdout);
...
//    arg[6] = last_str
    ...
    snprintf(outFile, MAX_FILENAME_LEN, "%s/scd_step_%s_%s_%s-%s.dat", argv[1], argv[2], argv[3], argv[5], argv[6]);
   ...
   // printf("Write SCD_\n");
    ...
    snprintf(outFile, MAX_FILENAME_LEN, "%s/scd_r_sn1_%s_%s_%s-%s.dat", argv[1], argv[2], argv[3], argv[5], argv[6]);
    fpOut = fopen(outFile,"w");
    for (i=1; i<NUM_C_TAIL; i++) {
    ...
    snprintf(outFile, MAX_FILENAME_LEN, "%s/scd_r_sn2_%s_%s_%s-%s.dat", argv[1], argv[2], argv[3], argv[5], argv[6]);
    fpOut = fopen(outFile,"w");
    for (i=1; i<NUM_C_TAIL; i++) {
        for (j=0; j<MAX_XY_DIST; j++) {
           // printf("%d %d %f %d\n", i, j, chain_scd_r[i][j],chain_scd_cnt_r[i][j]);
    ...
    snprintf(outFile, MAX_FILENAME_LEN, "%s/pc_scd_r_sn2_%s_%s_%s-%s.dat", 
    ...
    snprintf(outFile, MAX_FILENAME_LEN, "%s/pg_scd_r_sn2_%s_%s_%s-%s.dat",
      argv[1], argv[2], argv[3], argv[5], argv[6]);
    fpOut = fopen(outFile,"w");
    for (j=0; j<MAX_XY_DIST; j++) {
    ...
    snprintf(outFile, MAX_FILENAME_LEN, "%s/pc_scd_r_sn2_OPP_%s_%s_%s-%s.dat",
      argv[1], argv[2], argv[3], argv[5], argv[6]);
    fpOut = fopen(outFile,"w");
    for (j=0; j<MAX_XY_DIST; j++) {
    ...
    snprintf(outFile, MAX_FILENAME_LEN, "%s/pg_scd_r_sn2_OPP_%s_%s_%s-%s.dat",
      argv[1], argv[2], argv[3], argv[5], argv[6]);
    fpOut = fopen(outFile,"w");
    for (j=0; j<MAX_XY_DIST; j++) {
